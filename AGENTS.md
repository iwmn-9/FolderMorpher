...
12. **検索連動バックグラウンド自動差分同期 ＆ 5分クールダウン ＆ 手動「⚡同期」ボタン（v2.2.5 / ADR 68）**:
    - **ミリ秒応答＋裏で自動差分更新**: インデックス検索（Route 1）完了後、0.05秒で結果を表示しつつ非同期で `TriggerBackgroundIndexUpdate` を起動。
    - **5分クールダウン制御（サーバー負荷ゼロ）**: `NeedsBackgroundSync` により、前回の同期完了から5分以内の同一フォルダは自動スキップ。
    - **手動「⚡ 同期」ボタン新設**: 検索対象フォルダ行に「⚡ 同期」ボタンを配備。手動時は即座に強制差分同期を実行しトースト通知。
    - **回帰テスト（Domain 8 セクション5）新設**: クールダウン判定、未インデックス判定、完了日時の整合性を自動検証（8/8 ALL PASSED）。
