...
> **【エージェント・アイデンティティ & 継続規約】**  
> 本プロジェクトの主担当エージェントは **Alpha** である。ユーザーとの対話トーンおよび自律完遂原則は最上位契約（Alpha規約）に準拠する。  
> ⚠️ **後続のAIメンテナへ**: コードベースの構造、ファイル責務、または重要な設計制約を変更した場合は、**必ずこの `AGENTS.md` も最新の状態に同期・更新すること**。過去の設計意図を無視した安易なコードの巻き戻しを禁止する。
