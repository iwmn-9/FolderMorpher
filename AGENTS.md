...
17. **検索スタジオ ヘッダー整線（ピル撤去）＆ 5項目厳選ソート＆関連度スコアリング ＆ フォルダーベクターアイコン換装（v2.2.5 / ADR 73）**:
    ...
    - **フォルダーアイコンの「DIR」文字撤去 ＆ 洗練されたアンバーゴールド・ベクターフォルダー換装**: 四角い枠内に文字「DIR」と表示されていたバッジを撤去。検索結果一覧、容量分析（Tab 1）のツリービュー、選択フォルダーの内訳のすべてで、滑らかな角丸と上部タブを持つ上品なアンバーゴールド（`#FEF3C7` / `#FDE68A` / `#D97706`）のベクターフォルダーアイコンへ換装。
---
