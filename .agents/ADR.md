...
## 22. 【検索連動バックグラウンド自動差分同期 ＆ 5分クールダウン ＆ 手動「⚡同期」ボタン】
*(v2.2.5 本番施工 & ADR 68)*
...
- **施工内容**:
  ...
  2. **サーバー負荷ゼロの 5分クールダウン制御**:
     - 検索するたびに毎回ネットワーク走査が走るのを防ぐため、`ContentIndexService.NeedsBackgroundSync(folderPath, TimeSpan.FromMinutes(5))` を新設。
     - `IndexedRoots` の `LastCompletedUtcTicks` を最深Root基準で照合し、前回の同期完了から5分以内の場合は裏走査を完全スキップ。
  3. **手動「⚡ 同期」ボタンの新設 ＆ 多言語対応**:
     - 検索スタジオの対象フォルダー行（`SearchDirectBrowseButton` の右隣）に `SearchSyncIndexButton`（「⚡ 同期」/「⚡ Sync」）を配備。
     - 手動クリック時は `force: true` でクールダウンをバイパスして即座に差分同期を実行。「⚡ インデックス差分同期を開始しました...」➔ 完了時に「⚡ インデックスを最新化しました (追加/更新: 〇件, 削除: 〇件)」または「インデックスは既に最新です (差分なし)」をトースト通知。
  4. **自動回帰テストによる恒久保護**:
     - `RegressionTestSuite.Search.cs` にセクション5を新設。未インデックス時の同期要求判定、完了直後の5分クールダウン（同期抑止）、0秒指定時の即時判定、および `GetLastIndexCompletedUtc` の日時精度を自動検証。8/8 ALL PASSED を堅持。
