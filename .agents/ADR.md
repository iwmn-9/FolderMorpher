...
- **意味**: `AuditItem.ScoreBreakdown` を根拠の正本に保つ。完全重複の原本以外はSHA-256一致を95点、原本候補は保護対象として0点とし、両方に内訳を付ける。点数は整理候補の優先順位で、削除してよい確率ではない。削除可否は従来どおり `IsCleanable` と実行計画・再照合で判断する。
