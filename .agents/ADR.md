...
> **【AIメンテナ・自律継続規約】**  
> 本ドキュメントは、FolderMorpher の全 98 項目に及ぶ過去の設計判断・障害対策（ADR 1〜98）の正本記録である。  
> ⚠️ **後続のAIメンテナへ**:
...
> ⚠️ **【重要・後続変更】**: 本 ADR（および ADR 72〜86）で導入された検索専用 FTS5 DB や FileSystemWatcher は、後の **ADR 87（v2.2.14）において「ローカルDB肥大化・ロック競合ゼロ化」のため撤去** され、インメモリ検索＋Live直接走査（Channel パイプライン）＋WSPサーバー借用へ一本化されました。安易な巻き戻しを禁止します。
