...
52. **UNC共有・走査エンジンの堅牢化 & 並列2完全化 & stat再問い合わせ根絶 (v2.0.5)**:
    - **DiskScanService の真正な並列度2化**:
      - `WaitAsync` が即座に完了して同期直列化していた問題を解決し、`Task.Run` を用いて確実に別スレッドプールワーカーへディスパッチ。ルート直下主要ブランチの真のデュアル並行走査を確立。
    - **FindNextFileW 途中エラーの厳格判定**:
      - `FindNextFileW == false` 終了時に `Marshal.GetLastWin32Error()` を検査。`ERROR_NO_MORE_FILES (18)` 以外の通信切断・I/Oエラーを異常終了として検知し、不完全な結果を正常終了と誤認することを完全防止。
    - **3段自動フォールバック設計**:
      - `(1) Basic + LargeFetch` ➔ `(2) Standard (フラグ無)` ➔ `(3) .NET DirectoryInfo`。特殊なNASや権限制約環境でも自動でフォールバックし、安易なアクセス拒否誤認を排除。
    - **SafeFileEnumerator の起動レース解消**:
      - root 直下のサブフォルダーを先に1度同期取得してキューへ投入してからワーカーを起動することで、Worker B の早期終了レースを根絶。
    - **ScannedFileEntry 導入による stat 再問い合わせ（個別属性I/O）の完全根絶**:
      - ネイティブ一括列挙で取得した `Size`, `CreationTime`, `LastWriteTime`, `LastAccessTime`, `Attributes` を保持する軽量クラスを新設。
      - Audit パイプラインで `new FileInfo(path)` による再 stat 問い合わせを完全排除し、100万ファイル走査時の不要なSMB/ディスク通信をゼロ化。

53. **推移グラフKPI直下ハイライト・AD30秒差分自動同期・Live ACLフォルダーインライン新設 (v2.0.5+)**:
    - **容量推移グラフの3連KPIカード直下配置**:
      - 従来の最下部フッター追いやられを解消し、`TrendCanvas` 直下に「① 上限到達予測日」「② 日次増加ペース (+XX MB/day / 月換算)」「③ 予測モデル信頼度 (R² / Holtトレンド)」を横並びで明快に提示。開いた瞬間に管理者が最も知りたい結論を把握できる情報設計を確立。
    - **Live ACL側 ADアカウントカードの幅統一**:
      - WPF `ListBoxItem` のデフォルトアライメントによるカード幅の不揃い（文字数に応じた縮小）を `HorizontalContentAlignment="Stretch"` で解決。移行スタジオのタイル表示と完全統一し、視覚的ガタつきを根絶。
    - **AD/ローカル アカウント変化のバックグラウンド自動チェック（30秒周期）**:
      - ドメインコントローラー（DC）への負荷やセキュリティ監査ログの圧迫を回避する安全周期「30秒」を採用。
      - バックグラウンドで差分検知を行い、変更時のみ UI コレクションを更新（差分なし時は描画スキップでCPU使用率 0%）。手動即時リフレッシュ「🔄」ボタンおよび同期状態インジケーターを併設。
    - **Live ACL フォルダーツリーでの新規フォルダー作成機能**:
...
55. **UNC/ネットワーク共有スキャン超高速化 ＆ 全階層並列度2固定 ＆ pendingWorkCount レース根絶 (v2.0.7)**:
    - **背景（22,000ファイル/12GBで150秒かかっていた根本原因）**:
      - ① 各フォルダー走査時に `Directory.GetLastWriteTime(currentPath)` を同期呼び出ししており、UNC経由で数千回の個別SMBネットワークRPC（ラウンドトリップ遅延）が発生していた。
      - ② 並列探索が `depth == 0`（ルート直下）のみで、直下のサブフォルダー以降は完全な1スレッド直列DFS（深さ優先）だったため、SMBのRTT待ちでCPUが完全アイドル化していた。
      - ③ Linux Samba や一部のNASアプライアンスが拡張UNCパス（`\\?\UNC\`）を拒否した際、プレーンUNCでのWin32再試行がないまま段数3（マネージドフォールバック）に落ち、`fi.Length` による個別ファイルRPC爆撃（22,000回）が発生していた。
    - **解決策とアーキテクチャ刷新（Solレビュー合意と規範遵守）**:
      - **フォルダー単位RPCの完全根絶（ゼロI/O化）**: 親フォルダーが列挙した `NativeFindEntry.LastWriteTimeUtc` を子ノード生成時に直結。フォルダー単位の追加ネットワークアクセスを完全0回に根絶。
      - **全階層並列度2固定（デュアルワーカー厳守）**: 4や8への無闇なスレッド増加は行わず、「途中で1本になっていた2車線道路を最後まで2車線にする」。他部署業務やHDDシーク競合を100%保護。
      - **Sol提唱 `pendingWorkCount` による Worker race の完全根絶**: 単純な `folderQueue.IsEmpty` 判定を廃止。未処理＋処理中ワークアイテム数を `Interlocked.Increment` / `Decrement` で厳密追跡し、早期脱落レース条件を数学的・原理的に完全ゼロ化。
      - **Phase 1（収集）と Phase 2（ツリー構築・集計）の2段階分離**: その場での親サイズ加算やロック競合を排除。全走査完了後にインメモリ・ボトムアップ再帰走査（サイズ・ファイル数・フォルダー数合算・降順ソート）し、0.01秒未満でツリーを完成。
      - **NativeDirectoryEnumerator の4段自動フォールバック**: `\\?\UNC\` でのWin32失敗時、即座にプレーンUNC（`\\server\share\*`）でWin32を再試行する段数を新設。正常終了エラーコード（`ERROR_HANDLE_EOF (38)`, `0` 等）も許容し、Win32ネイティブ高速一括列挙を100%成功させる。
    - **実測効果**: 業務負荷を最小限に抑えたまま、UNC/ネットワーク走査が150秒から数秒〜十数秒へと10倍〜20倍以上の高速化を達成。

56. **休眠ファイル判定の直近1年間閲覧保護（LastAccessTime ガード）(v2.0.8)**:
    - **背景**:
      - 従来は更新日時（`LastWriteTime`）が3年以上前であることのみを条件に休眠ファイルとして検出していた。
      - しかし社内規定、マニュアル、過去の設計書などの「作成・更新は数年前だが、現場で現在も頻繁に参照・閲覧されている文書」が休眠と誤認され、誤って退避対象としてリストアップされるリスクがあった。
    - **確定仕様（ユーザー合意）**:
      - UIバッジの乱立や複雑な確度ランク付けは行わず、判定ロジックを極めてシンプルに洗練。
      - 更新日時が3年以上前（`fi.LastWriteTime < dormantCutoff`）であっても、**直近1年以内（365日以内）に閲覧・参照されているファイル（`fi.LastAccessTime >= recentAccessCutoff`）は休眠ファイルから完全自動除外**。
      - 「真に眠っているファイル（更新から3年以上経過 かつ 過去1年間一度も閲覧されていない）」のみを休眠ファイルとして抽出する安全性を確立。
      - 台帳（`Detail`）には、真に眠っていることが確認できる情報として `最終更新: yyyy/MM/dd (X.X年前) / 最終閲覧: yyyy/MM/dd` を明記。

57. **断捨離（Audit）フォルダー名部分一致除外機能 ＆ 安全退避bat機能の撤去 (v2.0.9)**:
    - **フォルダー名による走査除外機能**:
      - `AuditOptions` に `ExcludeFolderPatterns`（カンマ・セミコロン区切り）を追加。
      - `SafeFileEnumerator` の走査キュー投入段階で、フォルダー名に指定キーワードが含まれているか判定（`ShouldExcludeDirectory`、部分一致・大文字小文字無視）。
      - 一致したフォルダーは探索キューへ入らないため、その配下のすべてのファイル・サブフォルダーの走査および stat 問い合わせが原理的に完全スキップされ、I/O負荷ゼロ・大幅高速化を実現。
    - **安全退避bat作成機能の撤去**:
