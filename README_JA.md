...
### 1. 📊 容量分析 & 監視 (Storage Explorer - TreeSizeスタイル)
![Tab 0: 容量分析](docs/screenshots/tab0_storage.png)
- **⚡ 超高速NTFS MFT直読みスキャン & 3段ハイブリッド自動フォールバック (Experimental / 実験的機能)**: 管理者権限でのローカルNTFSドライブ（またはファイルサーバー上での直接実行時）において、ディスク管理領域（`$MFT`）を直接バイナリ解析し、数百万ファイルを数秒で一括インデックス。完全読み取り専用（`GENERIC_READ`・排他ロックなし）で業務影響ゼロ。UNC共有や一般権限時は自動で通常並行スキャンへシームレスにフォールバック。
...
- **🔍 ユーザー/グループ逆引き権限インスペクター（Canonical DACL評価・多重入れ子ADグループ完全対応）**: 対象ユーザーやセキュリティグループを選択し、ファイルサーバー全体を逆引きスキャン（探索階層は無制限/指定可能）。グループの多重ネスト（`LDAP_MATCHING_RULE_IN_CHAIN` / tokenGroups による深い階層所属）を本人ID汚染なしで完全解決。Windowsの正規DACL評価順序（子の明示的Allowは親の継承Denyより優先）を忠実に再現し、そのユーザーが「実際にアクセスできる全フォルダー」と権限種別（フルコン/変更/読取）、付与経路（直接 or 経由グループ＋入れ子深度）を完全可視化。UNC共有アクセス時のSMB共有注意喚起、エクスプローラー直行ハイパーリンク付きExcel監査台帳を1クリック生成。
- **Windows詳細セキュリティ完全準拠**: 基本6項目 ＋ 高度なアクセス許可14項目（走査/属性/所有権など）を完全双方向連動で編集（Allow および Deny ルールを標準ACL評価順序で正しくサポート）。
...
- **安全第一の運用ポリシー**: ツールによる自動削除は行わず、各部署提出用の**「棚卸し台帳（Excel/CSV）」**および**「安全退避（Archive移動）バッチ」**の生成に留めます（※重複原本は必ず現場に残す安全保護ガード付き）。
...
- **写真の視覚的ロスレス軽量化**: スマホ写真（JPEG）を長辺2560px・85%品質で直接上書き軽量化（**約90%の容量削減**）。PNGはアルファ透過を100%保持した最適化を実行。撮影日時・更新日時・作成日時・Exif・回転情報を可能な限り保持。
