...
- **⚡ 超高速NTFS MFT直読みスキャン & 3段ハイブリッド自動フォールバック**: 管理者権限でのローカルNTFSドライブ（またはファイルサーバー上での直接実行時）において、ディスク管理領域（`$MFT`）を直接バイナリ解析し、数百万ファイルを数秒で一括インデックス。完全読み取り専用（`GENERIC_READ`・排他ロックなし）で業務影響ゼロ。UNC共有や一般権限時は自動で通常並行スキャンへシームレスにフォールバック。
