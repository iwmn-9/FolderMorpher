...
- **🔍 ユーザー/グループ逆引き権限インスペクター（多重入れ子ADグループ完全対応）**: 対象ユーザーやセキュリティグループを選択し、ファイルサーバー全体を逆引きスキャン。グループの多重ネスト（`LDAP_MATCHING_RULE_IN_CHAIN` / tokenGroups による深い階層所属）を自動解決し、そのユーザーが「実際にアクセスできる全フォルダー」と権限種別（フルコン/変更/読取）、付与経路（直接 or 経由グループ＋入れ子深度）を完全可視化。エクスプローラー直行ハイパーリンク付きExcel監査台帳を1クリック生成。
